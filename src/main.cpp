...
{ //If the simulation is a transient simulation
		...
		if(nd.ID > 0)
{
	...
	if(_.start == _.end && _.end == 0)
{ //If the simulation is a dc simulation
		string s = "";
		...
		for(auto dS : _.dSources)
{
	...
	for(double time = _.start; time<_.end; time+=_.timeStep)
{ //for each timestep from the start to the end
