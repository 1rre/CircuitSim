...
{  //for each independent current or voltage source "iS" in the vector of sources
...
{ //if the time is less than 0 (ie -1), we are doing an operating point check
		...
		if(iS.cName == 'I')
{ //If the source "iS" is a current Source
...
{ //for each dependent current or voltage source "dS" in the vector of dependent sources
		double val;
		if(time>=0)
{
